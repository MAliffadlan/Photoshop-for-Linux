...
The importer accepts Compositor package versions 1–8, including Swift's alternating-key enum dictionaries, individual color channels, mask placement/link flags, grain parameters, live shape styles, alignment guides, layer effects, Photoshop's blend modes and the Black & White, Color Balance and Invert adjustments. Version 8 also covers Compositor 1.2.1 and 1.2.2, which kept the same format version. Import is one-way: Save creates a `.mectov` file and leaves the `.comp` package untouched.

## Guides and layer effects (version 3)

Documents carrying alignment guides or layer effects are written as version 3, so older readers do not silently drop them. The reader accepts versions 1–3; ordinary documents remain version 1, and documents with embedded RAW remain version 2. The adjustment kinds and blend modes added with Compositor 1.2.2 ride in the version their document already uses, as they do in `.comp` version 8: an older reader reports the unknown kind instead of dropping the layer.

An optional `guides` list holds the alignment guides a Compositor project placed: each entry has an `axis` (`Horizontal` or `Vertical`) and a `position` in document pixels, a Y for horizontal guides and an X for vertical ones. Canvas Size and Image Size move or scale them, Crop drops the ones pushed off the canvas, and Flip Canvas mirrors only the guides running perpendicular to the flip. Up to 1,024 guides are stored, and Clear Guides in the View menu removes them. Rulers, which would create guides, are not implemented, so guides currently arrive by import.

A layer's optional `effects` object preserves Compositor's layer effects: `stroke`, `shadow`, `color_overlay`, `inner_shadow` and `outer_glow`, each with its own geometry, color and opacity, plus an `enabled` flag that hides an effect rather than deleting it. Reading validates the source's ranges: sizes and blurs up to 500 px, shadow distances up to 5,000 px, angles within ±360°, and colors and opacities within 0–1. Effects stay with the layer through undo and project round trips and are not drawn yet.
