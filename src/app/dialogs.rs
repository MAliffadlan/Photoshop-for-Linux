    ...
    pub(super) fn dialogs(&mut self, ctx: &egui::Context) {
        ...
        if let Some(dialog) = self.dialog {
            match dialog {
                ...
                Dialog::About => {
                    let mut open = true;
                    widgets::Window::new("About mectov")
                        .open(&mut open)
                        .show(ctx, |ui| {
                            ui.heading("mectov");
    ...
    fn size_dialog(&mut self, ctx: &egui::Context, dialog: Dialog) {
        ...
        widgets::Window::new(title)
            ...
            .show(ctx, |ui| {
                ...
                let valid = mectov::document::validate_size(self.dimensions[0], self.dimensions[1]);
                ui.add_space(12.0);
                    ...
                    match adjustment {
                        Adjustment::HueRanges { settings } => {
                            widgets::PopUp::from_id_salt("hue_range")
                                .selected_text(mectov::color::HueSettings::RANGES[settings.range])
                                .show_ui(ui, |ui| {
                                    for (index, name) in
                                        mectov::color::HueSettings::RANGES.iter().enumerate()
                                    {
                        ...
                        Adjustment::BlackWhite {
                            ...
                            changed |= widgets::checkbox(ui, tint, "Tint").changed();
                            if *tint {
                                changed |= ui
                                    .add(
                        ...
                        Adjustment::ColorBalance {
                            ...
                            for (title, shift) in [
                                        ...
                                        .zip(shift.iter_mut())
                                {
                                    changed |= ui
                                        .add(
                                            widgets::Slider::new(value, -100.0..=100.0).text(label),
                                        )
                                        .changed();
                                }
                            }
                            changed |=
                                widgets::checkbox(ui, preserve_luminosity, "Preserve Luminosity")
                                    .changed();
                        }
                    }
                }
                    ...
                    let result = if let Some(effects) = &edit.layer_effects {
                        ...
                        if let Some(target) = edit.target {
                            ...
                            if session.document.selection.is_some() {
                                let mask =
                                    mectov::paint::mask_from_selection(&session.document, &layer);
                                layer.mask = Some(mectov::document::Mask {
                                    pixels: Arc::new(mask),
                                    ..mectov::document::Mask::white()
                                });
                            }
