...
Open a camera RAW file — Nikon `.nef`/`.nrw`, Canon `.cr2`/`.cr3`/`.crm`/`.crw`, Sony `.arw`/`.ari`, Fujifilm `.raf`, Panasonic `.rw2`, Olympus `.orf`, Pentax `.pef`, Samsung `.srw`, `.dng` and the other formats Rawler decodes — through File → Open, Import Image as Layer, the command line, a file-manager paste, or drag-and-drop. The image first opens in Develop. Files selected together are queued, so each RAW receives its own Develop session. Import-as-layer remembers its destination project.
