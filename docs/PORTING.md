...
The Linux port is implemented in Rust with egui 0.33 and wgpu 27. The source baseline is Compositor commit `a19db9011282399785dc18efcfded904627bdcc2` (Compositor 1.0.4), which writes `.comp` version 7. Later releases are ported on top of it, up to `609dbea` (Compositor 1.2.2, 2026-09-22): the version 8 manifest from 1.2.0 (`0a7342442223526fe7ac0bed7284f4c237d7d783`) with its alignment guides, layer effects and Soft Light; Photoshop's full blend menu from 1.2.2; Black & White, Color Balance and Invert from 1.2.2; and 1.2.1's wider camera RAW support and persisted tool settings. Neither source tree was changed by this port.
...
| `blend`, `render`, `gpu`, `composite.wgsl`, `mipmap.wgsl` | CPU reference/fallback compositor and wgpu compute preview/export, Photoshop's twenty-four blend modes, inherited masks and clipping, premultiplied Lanczos downsampling and preview mipmaps |
...
| `color`, `effects` | Selective hue ranges, RGB channel levels/curves, linear-light exposure, gradient mapping, film grain, Photoshop's Black & White and Color Balance, raster filters |
...
- `cargo test --locked --all-targets`: **141 tests pass** (72 library and 69 application tests); native GPU, interaction benchmark, sample-RAW, and system-clipboard checks remain opt-in.
...
- Version 8 projects import their guides and layer effects. Guides are drawn in cyan, follow canvas size/image size/crop/flip, and Clear Guides in the View menu removes them; rulers and the layout grid are not ported, so guides only arrive by import. Layer effects (stroke, drop shadow, color overlay, inner shadow, outer glow) are validated and preserved through saving, loading and undo, but not drawn yet.
- A Compositor line shape imports as its pixels, because mectov has no editable line shape yet; the layer stays plain pixels instead of redrawing as a rectangle. Photoshop PSD import, added in Compositor 1.2.0, is not ported. Brush smoothing (1.2.0), selection Expand/Contract (1.1.1), the Magic tool's Object mode, text paragraph boxes, and shortcut remapping are also still outstanding.
...
- Compositor 1.2.1's macOS-only fixes have no counterpart here: Cmd-A inside text fields, the app icon redrawn on Apple's grid, and Sparkle's update feed. Its middle-click pan, wider camera RAW support and remembered tool settings are ported; rulers, the layout grid and the Snap To options are not, so those settings have nothing to remember yet.
...
- One opt-in native check, `gpu::processing_tests::processing_raw_matches_cpu_at_both_depths`, currently reports a 185/65535 maximum difference with 14 sharpening-boundary pixels on this workstation's driver, which exceeds its eight-pixel outlier allowance. It fails identically at the commit before the 1.2.1/1.2.2 work, so it reflects the local graphics stack rather than these changes; the 8-bit comparison in the same test stays within one step, and the check passes on the CI runner.
