...
use crate::{
    document::{Document, GuideAxis, Layer, Point, Transform, validate_size},
    render, selection,
};
...
pub fn crop(document: &mut Document, start: Point, end: Point) -> Result<()> {
    ...
    // A guide the crop pushed off the canvas has nothing left to align against.
    let (right, bottom) = (width as f32, height as f32);
    document.guides.retain(|guide| match guide.axis {
        GuideAxis::Vertical => (0.0..=right).contains(&guide.position),
...
pub fn image_size(document: &mut Document, width: u32, height: u32) -> Result<()> {
    ...
    let (scale_x, scale_y) = (width as f32 / old.width, height as f32 / old.height);
    for guide in &mut document.guides {
        guide.position *= if guide.axis == GuideAxis::Vertical {
            scale_x
        } else {
...
pub fn flip_canvas(document: &mut Document, horizontal: bool) {
    ...
    // Only the guides running perpendicular to the flip swap sides.
    for guide in &mut document.guides {
        match (horizontal, guide.axis) {
            (true, GuideAxis::Vertical) => guide.position = document.width as f32 - guide.position,
            (false, GuideAxis::Horizontal) => {
                guide.position = document.height as f32 - guide.position;
            }
            _ => {}
        }
    }
...
mod tests {
    ...
    #[test]
    fn canvas_changes_keep_guides_on_their_content() {
        ...
        canvas_size(&mut doc, 30, 20, [0.5, 0.5]).unwrap();
        assert_eq!(doc.guides[0].position, 10.0);
        assert_eq!(doc.guides[1].position, 15.0);
        // The crop pushes the vertical guide off the canvas, which leaves it nothing to align against.
        crop(&mut doc, Point::new(12.0, 0.0), Point::new(30.0, 20.0)).unwrap();
        assert_eq!(
            doc.guides,
            [Guide {
                axis: GuideAxis::Horizontal,
        ...
        assert_eq!(doc.guides[1].position, 14.0);
        assert_eq!(doc.guides[0].position, 15.0);
        image_size(&mut doc, 36, 20).unwrap();
        assert_eq!(doc.guides[1].position, 28.0);
        assert_eq!(doc.guides[0].position, 15.0);
    }
