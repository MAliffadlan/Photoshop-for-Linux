...
produces no event that mectov can handle, including through eframe's raw input
hook.

The local patch emits `Event::Paste("")` when clipboard text is unavailable
or empty. mectov uses that event to read the image clipboard. egui text fields
