                ...
                ui.add_enabled_ui(active.is_some_and(|layer| !layer.group), |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        widgets::PopUp::from_id_salt("blend_mode")
                            ...
                            .show_ui(ui, |ui| {
                                for (index, group) in BlendMode::groups().iter().enumerate() {
                                    if index > 0 {
                                        widgets::menu_separator(ui);
                                    }
                                    for mode in group.iter().copied() {
                                        changed |=
    ...
    fn layer_thumbnail(
        ...
        let thumbnail = session.thumbnails.entry(key).or_insert_with(|| {
            let color = if mask {
                ...
                let image = mectov::render::render_thumbnail(
                    &thumbnail_document,
    ...
    fn reorder_layer(
        ...
        self.edit("Reorder Layer", |doc| {
            ...
            if duplicate {
                ...
                mectov::operations::duplicate(doc);
            }
