...
/// The extensions Compositor 1.2.1 opens — Canon, Nikon, Sony, Fujifilm, DNG and more. These are
/// exactly the formats rawler decodes; mectov picks the decoder from the file's contents, so this
/// list only decides which files reach Develop.
pub const RAW_EXTENSIONS: [&str; 25] = [
    "ari", "arw", "cr2", "cr3", "crm", "crw", "dcr", "dcs", "dng", "erf", "iiq", "kdc", "mef",
    "mos", "mrw", "nef", "nrw", "orf", "pef", "qtk", "raf", "rw2", "srw", "tfr", "x3f",
];

pub fn is_raw(path: &Path) -> bool {
    path.extension()
        .and_then(|s| s.to_str())
        .is_some_and(|s| RAW_EXTENSIONS.iter().any(|raw| s.eq_ignore_ascii_case(raw)))
}
...
fn decode_inner(bytes: &[u8]) -> Result<DecodedRaw> {
    ...
    validate_size(header.width.try_into()?, header.height.try_into()?)?;
        ...
        "This RAW sensor layout is not supported; an RGB Bayer sensor is required"
    );
