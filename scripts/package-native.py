...
def build_deb(payload, output, version, architecture, glibc):
    ...
    dynamic = subprocess.check_output(
        ["readelf", "--dynamic", payload / "usr/bin/mectov"], text=True
    )
    ...
    (control / "control").write_text(
        ...
        "Section: graphics\nPriority: optional\n"
        "Maintainer: M Alif Fadlan <maliffadlan@gmail.com>\n"
        "Homepage: https://github.com/MAliffadlan/Photoshop-for-Linux\n"
...
def build_rpm(payload, output, version, architecture, temporary):
    ...
    spec.write_text(
        ...
        "URL: https://github.com/MAliffadlan/Photoshop-for-Linux\n"
        "Packager: M Alif Fadlan <maliffadlan@gmail.com>\n"
        f"{requires}\nRecommends: xdg-desktop-portal\n"
        "\n%description\n"
        "Layered compositions, photo retouching, and camera RAW development.\n"
        "A Photoshop alternative for Linux.\n"
        '\n%install\nmkdir -p "%{buildroot}"\n'
        'cp -a "%{mectov_payload}/usr" "%{buildroot}/"\n'
        f"\n%post\n{refresh}\n%postun\n{refresh}"
        "\n%files\n%defattr(-,root,root,-)\n/usr/bin/mectov\n"
        "/usr/share/applications/me.silverl.mectov.desktop\n"
        "/usr/share/mime/packages/me.silverl.mectov.xml\n"
        "/usr/share/icons/hicolor/*/apps/me.silverl.mectov.png\n"
        "%doc /usr/share/doc/mectov\n%license /usr/share/licenses/mectov\n"
    )
    subprocess.run(
        [
            "rpmbuild",
...
def main():
    ...
    glibc = glibc_requirement(args.stage / "bin/mectov", architecture)
    ...
    with tempfile.TemporaryDirectory(prefix=".native-", dir=args.output) as directory:
        ...
        for package_format in formats:
            output = (
                args.output.resolve()
                / f"mectov-{args.version}-linux-{architecture}.{package_format}"
            )
