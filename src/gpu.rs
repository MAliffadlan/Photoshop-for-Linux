...
impl GpuCompositor {
    pub fn new(device: wgpu::Device, queue: wgpu::Queue) -> Self {
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("mectov layer compositor"),
        ...
        let mipmap_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("mectov preview mipmaps"),
    ...
    fn render_internal(
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("mectov composite frame"),
            });
        {
        ...
        for layer in render::paint_order(document) {
                ...
                self.sources.entry(key).or_insert_with(|| {
                    ...
                    let texture = self.device.create_texture_with_data(
                        ...
                        &wgpu::TextureDescriptor {
                            label: Some("mectov layer source"),
                            size: wgpu::Extent3d {
            ...
            let coverage = if accelerated_coverage.is_some() {
                ...
                Some(self.device.create_texture_with_data(
                    &self.queue,
                    &wgpu::TextureDescriptor {
                        label: Some("mectov mask coverage"),
                        size: wgpu::Extent3d {
                            width: size[0],
                            height: size[1],
                            depth_or_array_layers: 1,
    ...
    fn generate_mipmaps(&self, encoder: &mut wgpu::CommandEncoder) {
        for level in 1..self.display.mip_level_count() {
            ...
            let bind = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("mectov preview mipmap inputs"),
            ...
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("mectov preview mipmap"),
                timestamp_writes: None,
            });
    ...
    fn dispatch_to(
        ...
        let uniform = self
            ...
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("mectov layer parameters"),
        ...
        let bind = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("mectov composite inputs"),
...
fn target(
    ...
    device.create_texture(&wgpu::TextureDescriptor {
        label: Some("mectov canvas target"),
        size: wgpu::Extent3d {
            width: size[0],
            height: size[1],
            depth_or_array_layers: 1,
    ...
    if let Some(adjustment) = &layer.adjustment {
        match adjustment {
            ...
            Adjustment::BlackWhite {
                ...
                p.flags[1] = 12;
                p.first = [reds / 100.0, yellows / 100.0, greens / 100.0, cyans / 100.0];
                p.second = [
                    blues / 100.0,
                    magentas / 100.0,
                    *tint_hue,
                    if *tint { *tint_saturation / 100.0 } else { 0.0 },
                ];
            }
...
mod tests {
    ...
    fn gpu_matches_cpu_blending_adjustments_and_masks() {
        ...
        for mode in BlendMode::ALL {
            // Divide climbs without bound as the source approaches zero, so a rotated, warped edge
            // magnifies the premultiplied-versus-straight sampling difference between the paths.
            // It is compared on the flat opaque layer below instead.
            if mode == BlendMode::Divide {
                continue;
            }
        ...
        let mut flat = Document::new(12, 10).unwrap();
        flat.layers = vec![
            Layer::image(
                "Base",
                RgbaImage::from_fn(12, 10, |x, y| {
                    Rgba([40 + x as u8 * 12, 60 + y as u8 * 9, 120, 255])
                }),
            ),
            Layer::image(
                "Top",
                RgbaImage::from_fn(12, 10, |x, y| {
                    Rgba([90 + x as u8 * 7, 210 - y as u8 * 11, 150 + y as u8 * 8, 255])
                }),
            ),
        ];
        flat.layers[1].blend = BlendMode::Divide;
        compositor.render(&flat, [12, 10]);
        compare(&flat, &readback(&compositor), BlendMode::Divide.name());
        ...
        for effect in [
            ...
            Adjustment::BlackWhite {
                ...
                tint: true,
                tint_hue: 205.0,
                tint_saturation: 65.0,
            },
            Adjustment::neutral_color_balance(),
            Adjustment::ColorBalance {
                shadows: [30.0, -20.0, 0.0],
                midtones: [0.0, 25.0, -35.0],
                highlights: [-40.0, 0.0, 20.0],
