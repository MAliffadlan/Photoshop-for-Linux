...
- **Documents:** independent tab histories, crop with ratio presets, Trim, canvas/image size, high-quality downsampling, pixel grid, alignment guides imported from Compositor projects (shown in cyan, cleared with **View > Clear Guides**), pasting copied images or image files as layers, Copy Merged, and save-on-close prompts. Undo retains up to 64 steps with a 512 MiB asset budget, keeping at least one step.
